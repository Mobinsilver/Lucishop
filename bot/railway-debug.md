...
### 1. خطای "ImportError: cannot import name 'show_crypto_list' from partially initialized module"
**مشکل**: `ImportError: cannot import name 'show_crypto_list' from partially initialized module 'crypto_list' (most likely due to a circular import)`
**راه‌حل**: 
- Circular import بین `main.py` و `crypto_list.py` برطرف شده
- توابع `convert_price_for_user` و `get_usd_to_irr_rate` مستقیماً در `crypto_list.py` تعریف شده‌اند
- Import های مستقیم از `main.py` حذف شده‌اند

### 2. خطای "ValueError: too many values to unpack (expected 19)"
**مشکل**: `ValueError: too many values to unpack (expected 19)`
**راه‌حل**: 
- تعداد متغیرها در `admin_panel.py` از `range(20)` به `range(19)` تغییر یافته
- Constants ها در `main.py` از `admin_panel.py` import می‌شوند

### 3. خطای "ImportError: cannot import name 'get_store'"
