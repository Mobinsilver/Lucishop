...
async def admin_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
	"""دستور پنل مدیریتی"""
	await admin_panel.admin_panel_main(update, context)
...
def build_app():
	...
	app.add_handler(MessageHandler(filters.Regex("^🔐 پنل مدیریتی$"), admin_cmd))
	app.add_handler(MessageHandler(filters.Regex("^(👥 مدیریت ادمین‌ها|📢 ارسال همگانی|📝 تنظیم متن‌ها|🔧 مدیریت API ها|💰 مدیریت ارزها|📊 مدیریت شاخص‌ها|🔒 قفل اجباری|⚪ لیست سفید/سیاه|🗄️ مدیریت کش|⚙️ تنظیمات سیستم|💾 پشتیبان‌گیری|📋 لاگ‌ها|🔔 مدیریت هشدارها|📊 آمار و گزارش|⌨️ دستورات سفارشی|🤖 تنظیمات ربات|🔙 بازگشت به منو)$"), admin_panel.handle_admin_panel_text))
	app.add_handler(MessageHandler(filters.Regex("^(➕ افزودن ادمین|➖ حذف ادمین|📋 لیست ادمین‌ها|➕ افزودن API جدید|📋 لیست API ها|⚙️ تنظیمات API|➕ افزودن ارز|📋 لیست ارزها|⚙️ تنظیمات ارزها|➕ افزودن شاخص|📋 لیست شاخص‌ها|⚙️ تنظیمات شاخص‌ها|📌 پین کردن پیام همگانی)$"), admin_panel.handle_admin_panel_text))
	app.add_handler(MessageHandler(filters.Regex("^(🪙 کریپتو|💱 فیات|📰 اخبار|📊 تکنیکال|🔍 دریافت لیست ارزها از API|🔴 خاموش|🟢 روشن|🔙 بازگشت به لیست ارزها|⬅️ صفحه قبل|➡️ صفحه بعد)$"), admin_panel.handle_admin_panel_text))
	app.add_handler(MessageHandler(filters.Regex("^(⌨️ متن‌های کیبورد سریع|💬 متن‌های پیام‌ها|🎯 متن‌های قابلیت‌ها|📋 متن‌های منوها|⚠️ متن‌های خطا|ℹ️ متن‌های اطلاعاتی)$"), admin_panel.handle_admin_panel_text))
	app.add_handler(MessageHandler(filters.Regex("^(➕ افزودن چنل|➖ حذف چنل|📋 لیست چنل‌ها|⚙️ تنظیمات قفل|ادمین کردم)$"), admin_panel.handle_admin_panel_text))
	app.add_handler(MessageHandler(filters.Regex("^(📊 آمار 24 ساعت|📈 آمار 1 هفته|📅 آمار 1 ماه|🔄 بروزرسانی آمار|📋 گزارش کامل|⚙️ تنظیمات آمار)$"), admin_panel.handle_admin_panel_text))
	app.add_handler(MessageHandler(filters.Regex("^(🔧 مدیریت دکمه‌های کاربری|⚙️ مدیریت دکمه‌های ادمین|📊 آمار ویژگی‌ها|🔄 بازنشانی تنظیمات|🔄 تایید بازنشانی|❌ لغو)$"), admin_panel.handle_admin_panel_text))
	app.add_handler(MessageHandler(filters.Regex("^(➕ افزودن به لیست سیاه|➖ حذف از لیست سیاه|📋 لیست مسدودین|📊 آمار لیست‌ها|✅ تایید مسدودسازی|✅ تایید آزادسازی)$"), admin_panel.handle_admin_panel_text))
