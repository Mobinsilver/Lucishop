...
def reply_keyboard(update: Update = None) -> ReplyKeyboardMarkup:
	# دریافت وضعیت ویژگی‌ها
	features = STORE.get('bot_features', {})
	
	# دریافت متن‌های سفارشی کیبورد
	keyboard_texts = STORE.get('keyboard_texts', {})
	
	# تعریف دکمه‌ها بر اساس وضعیت ویژگی‌ها و متن‌های سفارشی
	keyboard = []
	
	# ردیف اول
	row1 = []
	if features.get('crypto_prices', True):
		crypto_text = keyboard_texts.get('crypto_prices', "💰 نرخ ارز")
		row1.append(KeyboardButton(crypto_text))
	if features.get('fiat_rates', True):
		fiat_text = keyboard_texts.get('fiat_rates', "🏦 ارز داخلی")
		row1.append(KeyboardButton(fiat_text))
	if row1:
	...
	if features.get('news', True):
		news_text = keyboard_texts.get('news', "📰 اخبار")
		row2.append(KeyboardButton(news_text))
	if features.get('charts', True):
		charts_text = keyboard_texts.get('charts', "📊 نمودار")
		row2.append(KeyboardButton(charts_text))
	if row2:
		keyboard.append(row2)
	
	# ردیف سوم
	row3 = []
	if features.get('technical_analysis', True):
		tech_text = keyboard_texts.get('technical_analysis', "📊 تحلیل تکنیکال")
		row3.append(KeyboardButton(tech_text))
	if features.get('arbitrage', True):
		arbitrage_text = keyboard_texts.get('price_comparison', "🔄 مقایسه قیمت‌ها")
		row3.append(KeyboardButton(arbitrage_text))
	if row3:
		keyboard.append(row3)
	
	# ردیف چهارم
	row4 = []
	if features.get('p2p', True):
		p2p_text = keyboard_texts.get('p2p', "💱 P2P")
		row4.append(KeyboardButton(p2p_text))
	if features.get('watchlist', True):
		watchlist_text = keyboard_texts.get('watchlist', "⭐ واچ‌لیست")
		row4.append(KeyboardButton(watchlist_text))
	if row4:
	...
	if features.get('portfolio', True):
		portfolio_text = keyboard_texts.get('portfolio', "💼 پرتفوی")
		row5.append(KeyboardButton(portfolio_text))
	if features.get('alerts', True):
		alerts_text = keyboard_texts.get('alerts', "🔔 هشدارهای من")
		row5.append(KeyboardButton(alerts_text))
	if row5:
	...
	if features.get('settings', True):
		settings_text = keyboard_texts.get('settings', "⚙️ تنظیمات")
		row6.append(KeyboardButton(settings_text))
	if features.get('help', True):
		help_text = keyboard_texts.get('help', "❓ راهنما")
		row6.append(KeyboardButton(help_text))
	if row6:
...
async def check_force_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
	...
	for channel_info in channels:
		if not channel_info.get('enabled', True):
			continue  # کانال غیرفعال است
			
		channel_link = channel_info.get('link', '')
		channel_title = channel_info.get('title', 'نامشخص')
		...
		try:
			...
			if channel_link.startswith('https://t.me/'):
				if '/joinchat/' in channel_link:
					# لینک خصوصی
					chat_id = channel_link.split('/joinchat/')[-1]
				else:
					# لینک عمومی
			...
			print(f"Error checking membership for {channel_title}: {e}")
			continue
...
async def show_force_subscription_message(update: Update, context: ContextTypes.DEFAULT_TYPE, channels: list):
	...
	for channel_info in channels:
		...
		channels_text += f"• {channel_title}\n"
		keyboard.append([InlineKeyboardButton(f"📢 عضویت در {channel_title}", url=channel_link)])
...
برای استفاده از ربات، باید در چنل‌های زیر عضو باشید:

{channels_text}

لطفاً در چنل‌های بالا عضو شوید و سپس دکمه "تایید عضویت" را بزنید.

⚠️ **نکته**: اگر عضو نشده‌اید، پیام هشدار 15 ثانیه‌ای نمایش داده می‌شود.
	"""
...
async def cancel_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
	...
	await update.message.reply_text("❌ عملیات لغو شد.", reply_markup=reply_keyboard(update))

async def notify_owner_new_user(update: Update, context: ContextTypes.DEFAULT_TYPE, user) -> None:
	"""اطلاع‌رسانی به مالک در مورد کاربر جدید"""
	# ذخیره اطلاعات کاربر در user_data
	user_id = user.id
	...
	# اطلاع‌رسانی به مالک و ادمین‌ها
	from admin_panel import admin_panel
	await admin_panel.notify_new_user_to_admins(context, user_id, username, first_name)
	
	# دکمه ارسال پیام (اختیاری - برای مالک)
	from config import OWNER_ID
	keyboard = [
		[InlineKeyboardButton("📩 پیام به کاربر", callback_data=f"message_user:{user_id}")]
	]
	reply_markup = InlineKeyboardMarkup(keyboard)
	
	# متن اطلاع‌رسانی اضافی برای مالک
	notification_text = f"""
🆕 **کاربر جدید ربات را استارت کرد!**

👤 **اطلاعات کاربر**:
• آیدی: `{user_id}`
• نام کاربری: @{username or 'بدون نام کاربری'}
• نام: {full_name or 'بدون نام'}
...
async def on_menu_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
	...
	# Handle menu buttons
	if text == "💰 قیمت ارز":
		if not features.get('crypto_prices', True):
			await update.message.reply_text("❌ این ویژگی توسط مدیر غیرفعال شده است.")
			return
		await show_crypto_list(update, context)
		return
	elif text == "🏦 ارز داخلی":
		if not features.get('fiat_rates', True):
			await update.message.reply_text("❌ این ویژگی توسط مدیر غیرفعال شده است.")
			return
		await show_domestic_currency_list(update, context)
		return
	elif text == "📰 اخبار":
		if not features.get('news', True):
			await update.message.reply_text("❌ این ویژگی توسط مدیر غیرفعال شده است.")
			return
		...
		if not features.get('charts', True):
			await update.message.reply_text("❌ این ویژگی توسط مدیر غیرفعال شده است.")
			return
		...
		if not features.get('technical_analysis', True):
			await update.message.reply_text("❌ این ویژگی توسط مدیر غیرفعال شده است.")
			return
		await update.message.reply_text("لطفا نماد ارز مورد نظر را وارد کنید (مثل: btc, eth)")
		ud = get_user_data(STORE, uid)
		...
		if not features.get('arbitrage', True):
			await update.message.reply_text("❌ این ویژگی توسط مدیر غیرفعال شده است.")
			return
		await update.message.reply_text("لطفا نماد ارز مورد نظر را وارد کنید (مثل: btc, eth)")
		ud = get_user_data(STORE, uid)
		...
		if not features.get('watchlist', True):
			await update.message.reply_text("❌ این ویژگی توسط مدیر غیرفعال شده است.")
			return
		...
		if not features.get('alerts', True):
			await update.message.reply_text("❌ این ویژگی توسط مدیر غیرفعال شده است.")
			return
		await show_alerts(update, context)
		return
	elif text == "🛠 تنظیمات":
		if not features.get('settings', True):
			await update.message.reply_text("❌ این ویژگی توسط مدیر غیرفعال شده است.")
			return
		await show_settings(update, context)
		return
	elif text == "❓ راهنما":
		if not features.get('help', True):
			await update.message.reply_text("❌ این ویژگی توسط مدیر غیرفعال شده است.")
			return
		...
		await admin_panel.admin_manage_admins(update, context)
		return
	elif text == "📢 ارسال همگانی":
		await admin_panel.broadcast_start(update, context)
		return
	elif text == "📝 تنظیم متن‌ها":
		await admin_panel.admin_texts_panel(update, context)
		return
	elif text == "🔧 مدیریت API ها":
		await admin_panel.manage_apis(update, context)
		return
	elif text == "💰 مدیریت ارزها":
		await admin_panel.manage_currencies(update, context)
		return
	elif text == "📊 مدیریت شاخص‌ها":
		await admin_panel.manage_indicators(update, context)
		return
	elif text == "🔒 قفل اجباری":
		await admin_panel.force_subscription_panel(update, context)
		return
	elif text == "⚪ لیست سفید/سیاه":
		await admin_panel.admin_lists_panel(update, context)
		return
	elif text == "🗄️ مدیریت کش":
		await admin_panel.admin_cache_panel(update, context)
		return
	elif text == "⚙️ تنظیمات سیستم":
		await admin_panel.admin_system_panel(update, context)
		return
	elif text == "💾 پشتیبان‌گیری":
		await admin_panel.admin_backup_panel(update, context)
		return
	elif text == "📋 لاگ‌ها":
		await admin_panel.admin_logs_panel(update, context)
		return
	elif text == "🔔 مدیریت هشدارها":
		await admin_panel.admin_alerts_panel(update, context)
		return
	elif text == "📊 آمار و گزارش":
		await admin_panel.admin_stats_panel(update, context)
		return
	elif text == "⌨️ دستورات سفارشی":
		await admin_panel.admin_commands_panel(update, context)
		return
	elif text == "🤖 تنظیمات ربات":
		await admin_panel.admin_bot_settings_panel(update, context)
		return
	elif text == "🔙 بازگشت به منو":
		await start(update, context)
		return
	elif text == "➕ افزودن ادمین":
		await admin_panel.add_admin_start(update, context)
		return
	elif text == "➖ حذف ادمین":
		await admin_panel.remove_admin_start(update, context)
		return
	elif text == "📋 لیست ادمین‌ها":
		await admin_panel.list_admins_action(update, context)
		return
	elif text == "➕ افزودن API جدید":
		await admin_panel.add_api(update, context)
		return
	elif text == "📋 لیست API ها":
		await admin_panel.list_apis_action(update, context)
		return
	elif text == "⚙️ تنظیمات API":
		await admin_panel.api_settings_panel(update, context)
		return
	elif text == "➕ افزودن ارز":
		await admin_panel.add_currency(update, context)
		return
	elif text == "📋 لیست ارزها":
		await admin_panel.list_currencies_action(update, context)
		return
	elif text == "⚙️ تنظیمات ارزها":
		await admin_panel.currency_settings_panel(update, context)
		return
	elif text == "➕ افزودن شاخص":
		await admin_panel.add_indicator(update, context)
		return
	elif text == "📋 لیست شاخص‌ها":
		await admin_panel.list_indicators_action(update, context)
		return
	elif text == "⚙️ تنظیمات شاخص‌ها":
		await admin_panel.indicator_settings_panel(update, context)
		return
	elif text == "📌 پین کردن پیام همگانی":
		await admin_panel.pin_broadcast_message(update, context)
		return
	elif text == "🪙 کریپتو":
		await admin_panel._process_api_type(update, context, "🪙 کریپتو")
		return
	elif text == "💱 فیات":
		await admin_panel._process_api_type(update, context, "💱 فیات")
		return
	elif text == "📰 اخبار":
		await admin_panel._process_api_type(update, context, "📰 اخبار")
		return
	elif text == "📊 تکنیکال":
		await admin_panel._process_api_type(update, context, "📊 تکنیکال")
		return
	elif text == "🔍 دریافت لیست ارزها از API":
		await admin_panel.fetch_currencies_from_api(update, context)
		return
	elif text == "🔴 خاموش":
		await admin_panel.process_currency_toggle(update, context)
		return
	elif text == "🟢 روشن":
		await admin_panel.process_currency_toggle(update, context)
		return
		...
		await admin_panel.process_currency_selection(update, context)
		return
	elif text == "➡️ صفحه بعد":
		await admin_panel.process_currency_selection(update, context)
		return
	elif text == "⌨️ متن‌های کیبورد سریع":
		await admin_panel.show_keyboard_texts_menu(update, context)
		return
	elif text == "💬 متن‌های پیام‌ها":
		await admin_panel.show_message_texts_menu(update, context)
		return
	elif text == "🎯 متن‌های قابلیت‌ها":
		await admin_panel.show_feature_texts_menu(update, context)
		return
	elif text == "📋 متن‌های منوها":
		await admin_panel.show_menu_texts_menu(update, context)
		return
	elif text == "⚠️ متن‌های خطا":
		await admin_panel.show_error_texts_menu(update, context)
		return
	elif text == "ℹ️ متن‌های اطلاعاتی":
		await admin_panel.show_info_texts_menu(update, context)
		return
	elif text == "➕ افزودن چنل":
		await admin_panel.add_force_sub_channel_start(update, context)
		return
	elif text == "➖ حذف چنل":
		await admin_panel.remove_force_sub_channel_start(update, context)
		return
	elif text == "📋 لیست چنل‌ها":
		await admin_panel.list_force_sub_channels(update, context)
		return
	elif text == "⚙️ تنظیمات قفل":
		await admin_panel.toggle_force_subscription(update, context)
		return
	elif text == "ادمین کردم":
		await admin_panel.process_admin_confirmation(update, context)
		return
	elif text == "📊 آمار 24 ساعت":
		await admin_panel.show_24h_stats(update, context)
		return
	elif text == "📈 آمار 1 هفته":
		await admin_panel.show_7d_stats(update, context)
		return
	elif text == "📅 آمار 1 ماه":
		await admin_panel.show_30d_stats(update, context)
		return
	elif text == "🔄 بروزرسانی آمار":
		await admin_panel.admin_stats_panel(update, context)
		return
	elif text == "📋 گزارش کامل":
		await admin_panel.show_full_report(update, context)
		return
	elif text == "⚙️ تنظیمات آمار":
		await admin_panel.toggle_live_notifications(update, context)
		return
	elif text == "🔧 مدیریت دکمه‌های کاربری":
		await admin_panel.manage_user_buttons(update, context)
		return
	elif text == "⚙️ مدیریت دکمه‌های ادمین":
		await admin_panel.manage_admin_buttons(update, context)
		return
	elif text == "📊 آمار ویژگی‌ها":
		await admin_panel.show_features_stats(update, context)
		return
	elif text == "🔄 بازنشانی تنظیمات":
		await admin_panel.reset_settings(update, context)
		return
	elif text == "🔄 تایید بازنشانی":
		await admin_panel.process_reset_confirmation(update, context)
		return
	elif text == "❌ لغو":
		await admin_panel.process_reset_confirmation(update, context)
		return
	elif text == "➕ افزودن به لیست سیاه":
		await admin_panel.add_to_blacklist_start(update, context)
		return
	elif text == "➖ حذف از لیست سیاه":
		await admin_panel.remove_from_blacklist_start(update, context)
		return
	elif text == "📋 لیست مسدودین":
		await admin_panel.show_blacklist_users(update, context)
		return
	elif text == "📊 آمار لیست‌ها":
		await admin_panel.show_lists_stats(update, context)
		return
	elif text == "✅ تایید مسدودسازی":
		await admin_panel.process_blacklist_confirmation(update, context)
		return
	elif text == "✅ تایید آزادسازی":
		await admin_panel.process_unblacklist_confirmation(update, context)
		return

	elif text == "🔔 هشدارهای من":
		await show_user_alerts_keyboard(update, context)
		return
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
	...
	if data.startswith("LANGSEL:"):
		...
		if all_subscribed:
			# کاربر در تمام کانال‌ها عضو است
			await query.edit_message_text(
				get_help_text(uid),
				reply_markup=reply_keyboard(update)
			)
		else:
			# کاربر هنوز عضو نشده - نمایش پیام هشدار 15 ثانیه‌ای
			warning_message = await query.message.reply_text(
				"⚠️ **عضو نشدی در چنل لطفا عضو شو بعد تایید بزن**",
				reply_to_message_id=query.message.message_id
			)
			
			# حذف پیام هشدار بعد از 15 ثانیه
			...
			# اجرای حذف در background
			context.job_queue.run_once(lambda ctx: asyncio.create_task(delete_warning()), 0)
			
			await query.answer("❌ هنوز در تمام چنل‌ها عضو نشده‌اید!")
		...
		if uid in alerts:
			...
			save_store(STORE)
		await query.edit_message_text(
			"✅ **همه هشدارها حذف شدند!**\n\n"
			"تمام هشدارهای شما پاک شدند.",
			reply_markup=InlineKeyboardMarkup([[
...
def build_app():
	...
	admin_conv_handler = ConversationHandler(
		...
		states={
			...
			ADD_API: [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_add_api),
				CommandHandler("cancel", admin_panel.cancel)
			],
			"SELECT_API_FOR_CURRENCIES": [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_api_selection_for_currencies),
				CommandHandler("cancel", admin_panel.cancel)
			],
			"CURRENCY_SELECTION": [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_currency_selection),
				CommandHandler("cancel", admin_panel.cancel)
			],
			"CURRENCY_TOGGLE": [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_currency_toggle),
				CommandHandler("cancel", admin_panel.cancel)
			],
			"KEYBOARD_TEXT_SELECTION": [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_keyboard_text_selection),
				CommandHandler("cancel", admin_panel.cancel)
			],
			"KEYBOARD_TEXT_EDIT": [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_keyboard_text_edit),
				CommandHandler("cancel", admin_panel.cancel)
			],
			"FORCE_SUB_ADMIN_CONFIRM": [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_admin_confirmation),
				CommandHandler("cancel", admin_panel.cancel)
			],
			"FORCE_SUB_LINK_INPUT": [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_channel_link),
				CommandHandler("cancel", admin_panel.cancel)
			],
			"USER_BUTTON_SELECTION": [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_user_button_selection),
				CommandHandler("cancel", admin_panel.cancel)
			],
			"USER_BUTTON_TOGGLE": [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_user_button_toggle),
				CommandHandler("cancel", admin_panel.cancel)
			],
			"ADMIN_BUTTON_SELECTION": [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_admin_button_selection),
				CommandHandler("cancel", admin_panel.cancel)
			],
			"ADMIN_BUTTON_TOGGLE": [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_admin_button_toggle),
				CommandHandler("cancel", admin_panel.cancel)
			],
			"RESET_CONFIRMATION": [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_reset_confirmation),
				CommandHandler("cancel", admin_panel.cancel)
			],
			"BLACKLIST_USER_INPUT": [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_blacklist_user_input),
				CommandHandler("cancel", admin_panel.cancel)
			],
			"BLACKLIST_CONFIRMATION": [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_blacklist_confirmation),
				CommandHandler("cancel", admin_panel.cancel)
			],
			"UNBLACKLIST_USER_INPUT": [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_unblacklist_user_input),
				CommandHandler("cancel", admin_panel.cancel)
			],
			"UNBLACKLIST_CONFIRMATION": [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_unblacklist_confirmation),
				CommandHandler("cancel", admin_panel.cancel)
			],
			ADD_CURRENCY: [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_add_currency),
				CommandHandler("cancel", admin_panel.cancel)
			],
			ADD_INDICATOR: [
				MessageHandler(filters.TEXT & ~filters.COMMAND, admin_panel.process_add_indicator),
				CommandHandler("cancel", admin_panel.cancel)
			]
		},
