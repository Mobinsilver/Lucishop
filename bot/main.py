...
def reply_keyboard(update: Update = None) -> ReplyKeyboardMarkup:
	...
	# تعریف دکمه‌ها بر اساس وضعیت ویژگی‌ها
	keyboard = []
	...
	if features.get('technical_analysis', True):
		row3.append(KeyboardButton("📈 تحلیل تکنیکال"))
	if features.get('arbitrage', True):
		row3.append(KeyboardButton("⚖️ مقایسه"))
	if row3:
	...
	if features.get('p2p', True):
		row4.append(KeyboardButton("🔄 P2P"))
	if features.get('watchlist', True):
		row4.append(KeyboardButton("👁 واچ‌لیست"))
	if row4:
	...
	if features.get('portfolio', True):
		row5.append(KeyboardButton("📚 پرتفوی"))
	if features.get('alerts', True):
		row5.append(KeyboardButton("🔔 هشدارها"))
	if row5:
	...
	if features.get('settings', True):
		row6.append(KeyboardButton("🛠 تنظیمات"))
	if features.get('help', True):
		row6.append(KeyboardButton("❓ راهنما"))
	if row6:
		keyboard.append(row6)
	
	# دکمه هشدارهای من (همیشه فعال)
	keyboard.append([KeyboardButton("🔔 هشدارهای من")])
...
• نام کاربری: @{username}
• نام: {full_name}
• تاریخ عضویت: {join_time}

📊 **آمار کلی**:
• کل کاربران: {len(STORE.get('users', [])):,}
• کاربران جدید امروز: {len([u for u in STORE.get('users', []) if STORE.get('user_data', {}).get(str(u), {}).get('join_date') == now.strftime('%Y-%m-%d')])}
	"""
