...
def reply_keyboard(update: Update = None) -> ReplyKeyboardMarkup:
	...
	# ردیف سوم
	row3 = []
	if admin_panel.is_feature_enabled('user.technical_analysis'):
		row3.append(KeyboardButton("📈 تحلیل تکنیکال"))
	if admin_panel.is_feature_enabled('user.arbitrage'):
	...
	# ردیف چهارم
	row4 = []
	if admin_panel.is_feature_enabled('user.p2p'):
		row4.append(KeyboardButton("🔄 P2P"))
	if admin_panel.is_feature_enabled('user.watchlist'):
	...
	# ردیف پنجم
	row5 = []
	if admin_panel.is_feature_enabled('user.portfolio'):
		row5.append(KeyboardButton("📚 پرتفوی"))
	if admin_panel.is_feature_enabled('user.alerts'):
...
async def on_menu_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
	...
	if text == "💰 قیمت ارز":
		...
		if not admin_panel.is_feature_enabled('user.technical_analysis'):
			await update.message.reply_text("این بخش غیرفعال است.")
			return
		await update.message.reply_text("لطفا نماد ارز مورد نظر را وارد کنید (مثل: btc, eth)")
		ud = get_user_data(STORE, uid)
		...
		if not admin_panel.is_feature_enabled('user.arbitrage'):
			await update.message.reply_text("این بخش غیرفعال است.")
			return
		await update.message.reply_text("لطفا نماد ارز مورد نظر را وارد کنید (مثل: btc, eth)")
		ud = get_user_data(STORE, uid)
		...
		if not admin_panel.is_feature_enabled('user.p2p'):
			await update.message.reply_text("این بخش غیرفعال است.")
			return
		...
		if not admin_panel.is_feature_enabled('user.watchlist'):
			await update.message.reply_text("این بخش غیرفعال است.")
			return
		await show_watchlist(update, context)
		return
	elif text == "📚 پرتفوی":
		if not admin_panel.is_feature_enabled('user.portfolio'):
			await update.message.reply_text("این بخش غیرفعال است.")
			return
		await show_portfolio(update, context)
		return
	elif text == "🔔 هشدارها":
		if not admin_panel.is_feature_enabled('user.alerts'):
			await update.message.reply_text("این بخش غیرفعال است.")
			return
...
async def main() -> None:
	...
	if webhook_url:
		...
		await telegram_app.run_polling(allowed_updates=Update.ALL_TYPES, close_loop=False)
