...
SUPPORTED_CRYPTOS = [
    ...
    {"symbol": "TRX", "name": "ترون", "emoji": "⚡"},
    {"symbol": "TON", "name": "تون", "emoji": "💎"},
    {"symbol": "BTC", "name": "بیتکوین", "emoji": "₿"},
    {"symbol": "ETC", "name": "اتریوم", "emoji": "🔶"},
...
async def show_crypto_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """نمایش لیست ارزها با پنل شیشه‌ای برای ارزهای اصلی"""
    # ارزهای اصلی که باید به صورت پنل شیشه‌ای نمایش داده شوند
    main_cryptos = [
        {"symbol": "BTC", "name": "بیتکوین", "emoji": "₿"},
        {"symbol": "ETH", "name": "اتریوم", "emoji": "Ξ"},
        {"symbol": "USDT", "name": "تتر", "emoji": "💵"},
        {"symbol": "TON", "name": "تون", "emoji": "💎"},
        {"symbol": "TRX", "name": "ترون", "emoji": "⚡"}
    ]
    
    # ایجاد پنل شیشه‌ای برای ارزهای اصلی
    keyboard = []
    row = []
    
    for i, crypto in enumerate(main_cryptos):
        # ایجاد دکمه شیشه‌ای برای هر ارز اصلی
    ...
    # دکمه‌های کنترلی
    keyboard.append([InlineKeyboardButton("📋 لیست کامل ارزها", callback_data="show_all_cryptos")])
...
• برای مشاهده لیست کامل ارزها، دکمه "لیست کامل ارزها" را بزنید
    """
...
async def show_all_cryptos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """نمایش لیست کامل ارزها با کیبورد سریع"""
    # استفاده از لیست کامل ارزها
    active_cryptos = SUPPORTED_CRYPTOS  # نمایش همه ارزها
    
    # ایجاد کیبورد سریع برای ارزها
    keyboard = []
    row = []
    
    # نمایش ارزهای اصلی در ردیف‌های 3 تایی
    ...
    # دکمه‌های کنترلی
    keyboard.append([KeyboardButton("🔍 جستجوی ارز")])
...
📊 **تعداد ارزهای فعال**: {len(active_cryptos)}
...
• از دکمه بازگشت برای بازگشت به منو استفاده کنید
    ...
    if update.callback_query:
        # برای callback query، فقط متن را تغییر بده و دکمه بازگشت اضافه کن
...
async def handle_crypto_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """مدیریت انتخاب ارز از کیبورد (فقط برای لیست کامل ارزها)"""
    text = update.message.text.strip()
    
    # بررسی اینکه آیا متن انتخاب شده یک ارز است
    for crypto in SUPPORTED_CRYPTOS:
        if f"{crypto['emoji']} {crypto['symbol']}" == text:
            # شبیه‌سازی callback query برای show_crypto_detail
            class MockCallbackQuery:
                def __init__(self, symbol):
                    self.data = f"crypto_detail:{symbol.lower()}"
            ...
            # ایجاد mock callback query
            mock_query = MockCallbackQuery(crypto['symbol'])
            ...
            mock_update = MockUpdate(mock_query, update.message)
            
            # فراخوانی show_crypto_detail
            await show_crypto_detail(mock_update, context)
            return
    
    # اگر ارز پیدا نشد، بررسی دکمه‌های کنترلی
    if text == "🔍 جستجوی ارز":
        await update.message.reply_text("🔍 **جستجوی ارز**\n\nلطفاً نام یا نماد ارز مورد نظر خود را تایپ کنید:\nمثال: bitcoin, btc, ethereum, eth")
        return
    elif text == "🔙 بازگشت به منو":
        # بازگشت به منوی اصلی
        from main import get_help_text, reply_keyboard
        await update.message.reply_text(
            get_help_text(update.effective_user.id),
            reply_markup=reply_keyboard(update)
        )
        return
    # اگر هیچ کدام از موارد بالا نبود، پیام خطا نده (ممکن است از منوی اصلی باشد)
...
async def show_crypto_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        # اگر از کیبورد آمده، نماد را از متن استخراج کن
        text = update.message.text.strip()
        for crypto in SUPPORTED_CRYPTOS:
            if f"{crypto['emoji']} {crypto['symbol']}" == text:
