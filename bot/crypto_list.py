...
async def show_crypto_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """نمایش لیست ارزها با دکمه‌های شیشه‌ای - فقط ارزهای فعال"""
    # دریافت ارزهای فعال از store
    store = load_store()
    enabled_currencies = store.get('enabled_currencies', {})
    
    # فیلتر کردن ارزهای فعال
    active_cryptos = []
    for symbol, currency_data in enabled_currencies.items():
        if currency_data.get('enabled', False) and currency_data.get('type') == 'crypto':
            # پیدا کردن emoji از لیست اصلی
            emoji = "🪙"  # emoji پیش‌فرض
            ...
            active_cryptos.append({
                'symbol': currency_data.get('symbol', symbol.upper()),
                'name': currency_data.get('name', symbol.upper()),
                'emoji': emoji
            })
    
    # اگر هیچ ارز فعالی وجود ندارد، از لیست پیش‌فرض استفاده کن
    if not active_cryptos:
        active_cryptos = SUPPORTED_CRYPTOS[:10]  # محدود به 10 ارز اول
