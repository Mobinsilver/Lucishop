import os
from dotenv import load_dotenv

# Load environment variables
load_dotenv()

# ===== اطلاعات ضروری ربات =====
TELEGRAM_BOT_TOKEN = os.getenv("BOT_TOKEN")
if not TELEGRAM_BOT_TOKEN:
    raise ValueError("BOT_TOKEN environment variable is required")

OWNER_ID = os.getenv("OWNER_ID")
if not OWNER_ID:
    raise ValueError("OWNER_ID environment variable is required")
try:
    OWNER_ID = int(OWNER_ID)
except ValueError:
    raise ValueError(f"OWNER_ID must be a valid integer, got: {OWNER_ID}")

ADMIN_ID = os.getenv("ADMIN_ID", str(OWNER_ID))
try:
    ADMIN_ID = int(ADMIN_ID)
except ValueError:
    ADMIN_ID = OWNER_ID  # Default to owner if invalid
BOT_USERNAME = os.getenv("BOT_USERNAME", "Crypto_navasan_bot")

# ===== تنظیمات Railway =====
WEBHOOK_URL = os.getenv("WEBHOOK_URL")
PORT = os.getenv("PORT", "8000")
try:
    PORT = int(PORT)
except ValueError:
    PORT = 8000  # Default port

# ===== تنظیمات محیط =====
ENVIRONMENT = os.getenv("ENVIRONMENT", "production")
LOG_LEVEL = os.getenv("LOG_LEVEL", "INFO")
DEBUG = os.getenv("DEBUG", "false").lower() == "true"

# ===== تنظیمات اختیاری =====
MAX_USERS = os.getenv("MAX_USERS", "1000")
try:
    MAX_USERS = int(MAX_USERS)
except ValueError:
    MAX_USERS = 1000

REQUEST_TIMEOUT = os.getenv("REQUEST_TIMEOUT", "30")
try:
    REQUEST_TIMEOUT = int(REQUEST_TIMEOUT)
except ValueError:
    REQUEST_TIMEOUT = 30

# ===== تنظیمات پیش‌فرض =====
DEFAULT_LANGUAGE = "FA"
DEFAULT_BASE_FIAT = "USD"
DEFAULT_SHOW_IRR = True
DEFAULT_DISPLAY_TOMAN = True

# ===== تنظیمات امنیتی =====
RATE_LIMIT_PER_MINUTE = 30
MAX_LOGIN_ATTEMPTS = 3

# ===== تنظیمات API =====
COINGECKO_API_URL = "https://api.coingecko.com/api/v3"
EXCHANGERATE_API_URL = "https://api.exchangerate.host"
BINANCE_API_URL = "https://api.binance.com/api/v3"

# ===== تنظیمات API جدید Tabdeal =====
TABDEAL_API_URL = "https://api1.tabdeal.org/r/api/v1"
TABDEAL_API_KEY = "einA3WTAOWYvNeBR9cdHIB6Tqbw3dkajeWJ8FSRqp3JHy5gr2STfoYMYWBXNa86X"
TABDEAL_API_SECRET = "TCzV5MY85pJe5O8NmhnsO6kGKo1X1jwIZm2XJifJtDbVaylEotj5TaNSykXnGWZP"

# ===== تنظیمات API جدید BRS (قیمت طلا و ارزهای داخلی) =====
BRS_API_URL = "https://brsapi.ir/Api/Panel"
BRS_API_KEY = "BcFwJ8XAXX2SixD6f5UXuIx3cA5b7CBq"

# ===== تنظیمات کش =====
CACHE_TTL_SECONDS = 180  # 3 minutes
MAX_CACHE_SIZE = 1000

# ===== تنظیمات اعلان‌ها =====
ALERT_CHECK_INTERVAL = 60  # 1 minute
MAX_ALERTS_PER_USER = 10

# ===== تنظیمات پورتفوی =====
MAX_WATCHLIST_ITEMS = 50
MAX_PORTFOLIO_ITEMS = 100

# ===== تنظیمات اخبار =====
NEWS_FEED_URLS = [
    "https://cointelegraph.com/rss",
    "https://coindesk.com/arc/outboundfeeds/rss/",
    "https://www.newsbtc.com/feed/"
]
MAX_NEWS_ITEMS = 10
