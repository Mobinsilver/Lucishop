...
class AdminPanel:
    ...
    async def handle_admin_panel_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if text == "👥 مدیریت ادمین‌ها":
            ...
            return await self.api_settings_panel(update, context)
            ...
            return await self.currency_settings_panel(update, context)
            ...
            return await self.indicator_settings_panel(update, context)
        elif text == "📌 پین کردن پیام همگانی":
            return await self.pin_broadcast_message(update, context)
        elif text == "🪙 کریپتو":
            return await self._process_api_type(update, context, "🪙 کریپتو")
        elif text == "💱 فیات":
            return await self._process_api_type(update, context, "💱 فیات")
        elif text == "📰 اخبار":
            return await self._process_api_type(update, context, "📰 اخبار")
        elif text == "📊 تکنیکال":
            return await self._process_api_type(update, context, "📊 تکنیکال")
        elif text == "🔍 دریافت لیست ارزها از API":
            return await self.fetch_currencies_from_api(update, context)
        elif text == "🔴 خاموش":
            return await self.process_currency_toggle(update, context)
        elif text == "🟢 روشن":
            return await self.process_currency_toggle(update, context)
        elif text == "🔙 بازگشت به لیست ارزها":
            return await self.process_currency_toggle(update, context)
        elif text == "⬅️ صفحه قبل":
            return await self.process_currency_selection(update, context)
        elif text == "➡️ صفحه بعد":
            return await self.process_currency_selection(update, context)
        elif text == "⌨️ متن‌های کیبورد سریع":
            return await self.show_keyboard_texts_menu(update, context)
        elif text == "💬 متن‌های پیام‌ها":
            return await self.show_message_texts_menu(update, context)
        elif text == "🎯 متن‌های قابلیت‌ها":
            return await self.show_feature_texts_menu(update, context)
        elif text == "📋 متن‌های منوها":
            return await self.show_menu_texts_menu(update, context)
        elif text == "⚠️ متن‌های خطا":
            return await self.show_error_texts_menu(update, context)
        elif text == "ℹ️ متن‌های اطلاعاتی":
            return await self.show_info_texts_menu(update, context)
            ...
            return await self.process_admin_confirmation(update, context)
        elif text == "📊 آمار 24 ساعت":
            return await self.show_24h_stats(update, context)
        elif text == "📈 آمار 1 هفته":
            return await self.show_7d_stats(update, context)
        elif text == "📅 آمار 1 ماه":
            return await self.show_30d_stats(update, context)
        elif text == "🔄 بروزرسانی آمار":
            return await self.admin_stats_panel(update, context)
        elif text == "📋 گزارش کامل":
            return await self.show_full_report(update, context)
        elif text == "⚙️ تنظیمات آمار":
            return await self.toggle_live_notifications(update, context)
        elif text == "🔧 مدیریت دکمه‌های کاربری":
            return await self.manage_user_buttons(update, context)
        elif text == "⚙️ مدیریت دکمه‌های ادمین":
            return await self.manage_admin_buttons(update, context)
        elif text == "📊 آمار ویژگی‌ها":
            return await self.show_features_stats(update, context)
        elif text == "🔄 بازنشانی تنظیمات":
            return await self.reset_settings(update, context)
        elif text == "🔄 تایید بازنشانی":
            return await self.process_reset_confirmation(update, context)
        elif text == "❌ لغو":
            return await self.process_reset_confirmation(update, context)
            ...
            return await self.show_blacklist_users(update, context)
        elif text == "📊 آمار لیست‌ها":
            return await self.show_lists_stats(update, context)
        elif text == "✅ تایید مسدودسازی":
            return await self.process_blacklist_confirmation(update, context)
        elif text == "✅ تایید آزادسازی":
            return await self.process_unblacklist_confirmation(update, context)
        else:
...
🔙 برای لغو: /cancel
        ...
        await update.message.reply_text(text, reply_markup=reply_markup)
        return "FORCE_SUB_ADMIN_CONFIRM"
...
🔙 برای لغو: /cancel
    ...
    async def process_channel_link(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not (channel_link.startswith('https://t.me/') or channel_link.startswith('@')):
            await update.message.reply_text("❌ فرمت لینک نامعتبر! لطفاً لینک معتبر ارسال کنید.")
            return "FORCE_SUB_LINK_INPUT"
...
• برای لغو عملیات: /cancel
    ...
    async def _process_api_url(self, update: Update, context: ContextTypes.DEFAULT_TYPE, url: str):
        ...
        if not url.startswith(('http://', 'https://')):
            await update.message.reply_text(
                "❌ **خطا در آدرس API!**\n\n"
                "لطفاً آدرس کامل API را وارد کنید (شامل http یا https)\n"
                "مثال: `https://api.coingecko.com/api/v3`"
            )
            return ADD_API
