...
class AdminPanel:
    ...
    async def handle_admin_panel_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if text == "👥 مدیریت ادمین‌ها":
        ...
        elif text == "تنظیمات API":
            await self.external_api_config_start(update, context)
            ...
            await self.enhanced_feature_toggle_menu(update, context)
    ...
    async def admin_panel_main(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # ساخت کیبورد بر اساس ویژگی‌های فعال
        keyboard = []
        ...
        row3 = []
        if self.is_feature_enabled('admin.api_management'):
            row3.append(KeyboardButton("تنظیمات API"))
        ...
        if self.is_feature_enabled('admin.indicators'):
            row4.append(KeyboardButton("📊 مدیریت شاخص‌ها"))
        ...
        if self.is_feature_enabled('admin.lists'):
            row5.append(KeyboardButton("⚪ لیست سفید/سیاه"))
        if self.is_feature_enabled('admin.cache'):
            row5.append(KeyboardButton("🗄️ مدیریت کش"))
        if row5:
            keyboard.append(row5)
        
        # ردیف 6
        row6 = []
        if self.is_feature_enabled('admin.system_settings'):
            row6.append(KeyboardButton("⚙️ تنظیمات سیستم"))
        if self.is_feature_enabled('admin.backup'):
            row6.append(KeyboardButton("💾 پشتیبان‌گیری"))
        if row6:
            keyboard.append(row6)
        
        # ردیف 7
        row7 = []
        if self.is_feature_enabled('admin.logs'):
            row7.append(KeyboardButton("📋 لاگ‌ها"))
        if self.is_feature_enabled('admin.alerts'):
            row7.append(KeyboardButton("🔔 مدیریت هشدارها"))
        if row7:
        ...
        if self.is_feature_enabled('admin.stats'):
            row8.append(KeyboardButton("📊 آمار و گزارش"))
        if self.is_feature_enabled('admin.commands'):
            row8.append(KeyboardButton("⌨️ دستورات سفارشی"))
        if row8:
            keyboard.append(row8)
        
        # ردیف 9
        row9 = []
        if self.is_feature_enabled('admin.bot_settings'):
            row9.append(KeyboardButton("🤖 تنظیمات ربات"))
        if row9:
