...
class AdminPanel:
    ...
    async def handle_admin_panel_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Admin panel now uses callbacks only - no text handling needed
        # If user sends text, show them the admin panel again
        print(f"DEBUG: User {user_id} sent text in admin panel, showing panel again")
    ...
    async def admin_panel_main(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # ایجاد کیبورد اینلاین به جای ReplyKeyboard
        from telegram import InlineKeyboardMarkup, InlineKeyboardButton
        
        # مقداردهی اولیه registry
        self.initialize_feature_registry()
        
        # ساخت کیبورد اینلاین بر اساس ویژگی‌های فعال
        keyboard = []
        
        # ردیف 1
        row1 = []
        if self.is_feature_enabled('admin.user_management'):
            row1.append(InlineKeyboardButton("👥 مدیریت ادمین‌ها", callback_data="adm:manage"))
        if self.is_feature_enabled('admin.broadcast'):
            row1.append(InlineKeyboardButton("📢 ارسال همگانی", callback_data="adm:broadcast"))
        if row1:
        ...
        if self.is_feature_enabled('admin.text_settings'):
            row2.append(InlineKeyboardButton("📝 تنظیم متن‌ها", callback_data="adm:texts"))
        if self.is_feature_enabled('admin.api_management'):
            row2.append(InlineKeyboardButton("🔧 مدیریت API ها", callback_data="adm:api"))
        if row2:
        ...
        if self.is_feature_enabled('admin.currencies'):
            row3.append(InlineKeyboardButton("💰 مدیریت ارزها", callback_data="adm:currencies"))
        if self.is_feature_enabled('admin.indicators'):
            row3.append(InlineKeyboardButton("📊 مدیریت شاخص‌ها", callback_data="adm:indicators"))
        if row3:
        ...
        if self.is_feature_enabled('admin.force_subscription'):
            row4.append(InlineKeyboardButton("🔒 قفل اجباری", callback_data="adm:force_sub"))
        if self.is_feature_enabled('admin.lists'):
            row4.append(InlineKeyboardButton("⚪ لیست سفید/سیاه", callback_data="adm:lists"))
        if row4:
            keyboard.append(row4)
        
        # ردیف 5 - حذف شده (مدیریت کش، تنظیمات سیستم، لاگ‌ها، مدیریت هشدارها، دستورات سفارشی، تنظیمات ربات، پشتیبان‌گیری)
        
        # ردیف 5
        row5 = []
        if self.is_feature_enabled('admin.stats'):
            row5.append(InlineKeyboardButton("📊 آمار و گزارش", callback_data="adm:stats"))
        if row5:
            keyboard.append(row5)
        
        # دکمه بازگشت (همیشه موجود)
        keyboard.append([InlineKeyboardButton("🔙 بازگشت به منو", callback_data="adm:back")])
...
**ویژگی‌های موجود**:
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Handle admin panel callbacks with adm: prefix
        if data.startswith("adm:"):
            action = data.split(":")[1]
            print(f"DEBUG: Admin panel action: {action}")
            ...
            if action == "back":
                ...
                print(f"DEBUG: Unknown admin action: {action}")
